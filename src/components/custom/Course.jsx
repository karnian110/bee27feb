  ...
  return (
    <section className="relative py-20 lg:py-28 bg-white overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Courses Grid - All Coming Soon */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {courses.map((course, idx) => (
            <div
              key={idx}
              className="group relative bg-white rounded-2xl shadow-sm shadow-slate-200/50 border border-slate-100 overflow-hidden"
            >
              {/* Card Top Accent */}
              ...
              <div className="p-5">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="p-2.5 bg-[#950E1D]/10 rounded-xl">
                    <Clock className="w-5 h-5 text-[#950E1D]" />
                  </div>
                  <span
                    className={`px-2.5 py-1 text-xs font-medium rounded-lg border ${getLevelColor(
                      course.level
                    )}`}
                  >
                    {course.level}
                ...
                {/* Title */}
                <h3 className="text-lg font-bold text-[#1B263B] mb-2 leading-snug">
                  {course.title}
                </h3>
                ...
                <Button
                  disabled
                  variant="outline"
                  className="rounded-lg w-full h-9 border-slate-200 text-slate-400 bg-slate-50 cursor-not-allowed"
                >
                  Coming Soon
                </Button>
              </div>
              ...
              <div className="absolute inset-0 bg-[#1B263B]/95 backdrop-blur-sm flex flex-col items-center justify-center text-center p-4 opacity-0 group-hover:opacity-100 transition-all duration-300">
                <div className="p-4 bg-amber-500/20 rounded-2xl border border-amber-500/30 mb-4">
                  <Construction className="w-10 h-10 text-amber-400" />
                </div>
                <h4 className="text-xl font-bold text-white mb-2">Coming Soon</h4>
                <p className="text-sm text-slate-300 mb-4">Stay tuned for updates!</p>
                <Button
                  asChild
                  size="sm"
                  className="rounded-lg bg-amber-500 hover:bg-amber-400 text-[#1B263B] font-medium"
                >
                  <Link href="#notify" className="gap-1.5">
